...
def frame_pattern(
    ...
    for mddEntry in mddData:
        ...
        for segDataLine in fasmLines:
            if re.search(mddEntry.tile + "\\.", segDataLine) != None and re.search("\.INIT_", segDataLine) != None:
                hexStringList = list(re.search("\'.*", segDataLine).group(0)[2:])
                hexStringList.reverse()
                basicInitStrings.append(hexStringList)
                if re.search("Y1\.INIT_", segDataLine) != None:
                    isRAMB36 = True
            elif re.search(mddEntry.tile + "\\.", segDataLine) != None and re.search("\.INITP_", segDataLine) != None:
                hexStringList = list(re.search("\'.*", segDataLine).group(0)[2:])
                hexStringList.reverse()
            ...
            if initRegexResult != None:
                ...
                if initStringIndex < numOfInitStrings:
                    if BRAMSide == "Y1" and isRAMB36:
                        basicSegData[initStringIndex + 64][bitIndex] = segDataLine
                    elif BRAMSide == "Y0":
                        basicSegData[initStringIndex][bitIndex] = segDataLine
            elif initPRegexResult != None:
                BRAMSide = re.search("Y[01]", segDataLine).group(0)
                initStringIndex = int(initPRegexResult.group(0)[7:].lower(), 16)
                bitIndex = int(re.search("\[...", segDataLine).group(0)[1:])
                if initStringIndex < len(initPStrings):
                    if BRAMSide == "Y1" and isRAMB36:
                        initPSegData[initStringIndex + 8][bitIndex] = segDataLine
                    elif BRAMSide == "Y0":
        ...
        for i in range(len(basicSegData)):
            for j in range(len(basicSegData[i])):
                segDataRegexResult = re.search(" .*_.*", basicSegData[i][j]).group(0)
                ...
                if j < len(basicInitStrings[i]):
                    reorderedBits[initStringIndex][bitIndex] = str(basicInitStrings[i][j])

        # Same as above but done for the INITP strings
        for i in range(len(initPSegData)):
            for j in range(len(initPSegData[i])):
                segDataRegexResult = re.search(" .*_.*", initPSegData[i][j]).group(0)
                dataBreak = segDataRegexResult.find('_')
                initStringIndex = int(segDataRegexResult[1:dataBreak])
                bitIndex = int(segDataRegexResult[dataBreak + 1:])
                if j < len(initPStrings[i]):
                    reorderedBits[initStringIndex][bitIndex] = str(initPStrings[i][j])
        ...
        for bitList in reorderedBits:
            ...
            for i in range(len(hexString), 0, -1 * WORD_LENGTH):
                if i == 0:
                    ...
                    hexStringList.append("0x" + ("0" * (WORD_LENGTH - i)) + hexString[0:i].upper())
                else:
                    hexStringList.append("0x" + hexString[i - WORD_LENGTH: i].upper())
        ...
        for i in range(len(frameStrings)):
            ...
            formattedAddress = "0x" + ("0" * (WORD_LENGTH - len(addressHex))) + addressHex
...
if __name__ == '__main__':
    if len(sys.argv) == 8:
        frame_pattern(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7])
    else:
        print("Usage: fasmFile dbFile memoryToRead mddFile jsonFile frmFile outfile")