...
if __name__ == "__main__":
    ...
    for lin in mlines:
        ...
        m = re.search('^init.mem\[(\d+)\]\[(\d+)\] -> [^.]+[^Y]+Y(.).INIT([^_]*)_(..).(...). -> [^ ]+ ([^ ]+) ([^_]+)_([^ ]+) wordoffset = ([\d]+)',
        lin.strip()
        )
        ...
        if m:
            ...
            tmp = [word, bit, ynum, parity, initline, initbit, frame, frameoffset, bitoffset, wordoffset, wordoffset*32+bitoffset]
            #print(tmp)
            mappings.append(tmp)
    ...
    print(res)
    for r in res:
        print("Y{} {}_{:02x}[{}] = bit {}".format(r[0], r[1], r[2], r[3], r[4]))
