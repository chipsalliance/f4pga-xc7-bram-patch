...
def init2fasm(
    ...
    for mapping in mappings:
        ...
        if mapping.fasmINITP:
            ...
            index_string += 'INIT'
        while len(reassembled_init_array[mapping.tile][index_string]
                  ) <= mapping.fasmLine:
            reassembled_init_array[mapping.tile][index_string].append(
                [None] * 256
            )
        bin_word = (bin(int(init_words[mapping.word], 16))[2:])[::-1]
        if mapping.bit >= len(bin_word):
            reassembled_init_array[mapping.tile][index_string][
                mapping.fasmLine][mapping.fasmBit] = '0'
        else:
            reassembled_init_array[mapping.tile][index_string][
                mapping.fasmLine][mapping.fasmBit] = bin_word[mapping.bit]
    ...
    for tile in reassembled_init_array:
        for group in reassembled_init_array[tile]:
            for line in reassembled_init_array[tile][group]:
                ...
                reassembled_init_strings[tile][group].append(
                    full_word[full_word.index('1'):]
                )
        ...
        for group in reassembled_init_strings[tile]:
            for i in range(len(reassembled_init_strings[tile][group])):
                ...
                for j in range(len(fasm_lines)):
                    if re.search(tile + "\.RAMB18_" + group[0:2] + "\." +
                                 group[2:] + "_0?" + fasm_num.upper() + "\[",
                                 fasm_lines[j]) != None:
                        k = j
                        break
                new_fasm_line = "{}.RAMB18_{}.{}_{}{}[{}:0] = {}'b{}\n".format(
                    tile, group[0:2], group[2:],
                    "" if len(fasm_num) == 2 else "0", fasm_num.upper(),
                    str(len(string) - 1), len(string), string
                )
                if k != -1:
                    ...
                    for j in range(len(fasm_lines)):
                        if re.search(tile + "\.RAMB18_" + group[0:2] + "\." +
                                     group[2:] + "_", fasm_lines[j]) != None:
                            if fasm_int == 0:
                                fasm_lines.insert(j, new_fasm_line)
                                break
                            if fasm_int > int(re.search(
                                    "_..\[", fasm_lines[j]).group(0)[1:3], 16):
                                entry_group = re.search(
                                    tile + "\.RAMB18_" + group[0:2] + "\." +
                                    group[2:] + "_..", fasm_lines[j + 1]
                                )
                                if entry_group == None:
    ...
    if checkFile != None:
        ...
        if len(check_lines) != len(fasm_lines):
            print(
                "  There is a different number of fasm lines in the original and new file"
            )
            exit(1)
...
if __name__ == "__main__":
    ...
    init2fasm(
        baseDir, args.memname, baseDir / args.mddname, baseDir / args.memfile,
        baseDir / "real.fasm",
        baseDir / "real.fasm" if args.check == True else None, args.verbose,
        args.printmappings, args.partial
    )
